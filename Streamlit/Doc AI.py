"""
Project Name: DOC AI
Module Name and Version: DOC AI v1.0
...
class GrammarCorrectorForm:
    ...
    def EstimateTokenCount(self, isoText):
        if not isoText:
            return 0
        return len(isoText) // 4 + 1  
...
class SafeDocxCreatorForm:
    ...
    def CreateTextFile(self, isoCorrectedText):
        isoTextIo = io.BytesIO()
        isoTextIo.write(isoCorrectedText.encode('utf-8'))
        isoTextIo.seek(0)
        return isoTextIo
...
class DocumentProcessorForm:
    ...
    def ProcessDocument(self, isoUploadedFile, isoCompatibilityMode="Safe Mode", isoProgressCallback=None, isoAdditionalInstructions=""):
        ...
        try:
            ...
            isoIsTextFile = isoUploadedFile.name.lower().endswith('.txt')
            isoIsDocx = isoUploadedFile.name.lower().endswith('.docx')
            if isoIsTextFile:
                isoCompatibilityMode = "Ultra Safe Mode"
            ...
            if isoIsDocx and isoCompatibilityMode == "Preserve All":
                ...
                if isoIsTextFile:
                    if isoProgressCallback:
                        isoProgressCallback(0.8, "Creating corrected text file...")
                    isoTextIo = self.docxCreator.CreateTextFile(isoCorrectedText)
                    isoResults['Corrected_txt'] = isoTextIo.getvalue()
    ...
    def RenderResultsPage(self, isoConfig):
        ...
        with isoTabs[0]:
            ...
            with isoCol1:
                ...
                """, unsafe_allow_html=True)
                if st.session_state.filename.lower().endswith('.txt') and isoResults.get('Corrected_txt') is not None:
                    st.download_button(
                        label="📝 Download Corrected TXT Document",
                        data=isoResults['Corrected_txt'],
                        file_name=f"Corrected_{st.session_state.filename}",
                        mime="text/plain",
                        key=isoDownloadKey,
                        help="Corrected text file in the original .txt format"
                    )
                    st.download_button(
                        label="📄 Download as DOCX (Alternative Format)",
                        data=isoResults['Corrected_docx'],
                        file_name=f"Corrected_{st.session_state.filename.replace('.txt', '.docx')}",
                        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                        key=f"docx_alt_download_{st.session_state.download_count}",
                        help="Alternative format that can be opened in Word"
                    )
                else:
                    st.download_button(
                ...
                if st.session_state.filename.lower().endswith('.txt'):
                    st.info("Note: The text file has been corrected while preserving the original format.")
                elif isoConfig['compatibility_mode'] == "Preserve All":
